...
const CreatePostCardExtended = ({
  closePostCardExtended, jwt
}: {
  closePostCardExtended: any, jwt:string
}) => {
      ...
      <div className="w-2/3 md:w-1/2 lg:w-1/3 bg-white rounded-xl space-y-6">
        ...
        <div className="relative">
          ...
          <button className="absolute bottom-0 right-5 text-2xl text-gray-400 overflow-hidden" >
            ...
            <input type ="file" className="absolute top-0 right-0 w-[500px] cursor-pointer" onClick={()=>{}}/>
          </button>
        </div>
        <button className="bg-purple-500 w-full py-3 font-bold text-white rounded-b-xl" onClick={handleSubmit}>
          Post Now
        </button>
      </div>
    </div>
